...
for pb_size in $PROBLEM_SIZE; do
    echo "Problem size: ${pb_size}, Snapshot Frequency: ${SNAPSHOT_FREQUENCY}, Compare with slices: ${COMPARE_WITH_SLICES}"
      ...
      "$EXECUTABLE_PATH" --ex ${pb_size} --ey ${pb_size} --ez ${pb_size} --timemax ${MAX_TIME} --snapshot true --save-interval $SNAPSHOT_FREQUENCY --sismos true --sismos-folder "${FOLDER}" --snap-folder "${FOLDER}" --slices > "${FOLDER}/output_snapshots_slice_${pb_size}_${SNAPSHOT_FREQUENCY}"
