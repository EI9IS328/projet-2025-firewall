...
Lors du dernier cours, nous avions présenté nos résultats dans une courte présentation en fin de scéance. Nous avons décidé de garder les mêmes figures. Cependant, pour les 
graphiques que nous avons générés pour ce rapport nous avons changé l'axe des absisses. Avant, nous présentions le nombre de \textit{Nodes} alors que nous avons choisi d'uitliser
la taille du domaine dans les derniers graphiques. Les deux notations sont équivalente car tous les points des graphiques correspondent aux tailles 50 à 150 avec un incrément de 10 
entre chaque point. Nous avons laissé les graphiques avec l'ancienne notation pour correspondre à notre présentation.
...
in-situ à un processus séparé, voire à des nœuds dédiés, mais nous avons préféré rester sur l'intégration directe, en in-line.
...
de la pression. Nous donnons tout de même les pressions enregistrées aux récepteurs des sismogrammes, mais nous ne faisons pas de traitement particulier.
...
Nous pouvons gérer presque toutes nos fréquences de sortie, que ce soit les snapshots, les slices ou les analyses in-situ.
...
\subsection{Snaphots}
\subsubsection{Snapshots globales}\ref{snapshots}
...
donné du maillage. Nous avons choisi de stocker ces informations sous forme de texte avec des fichiers de la forme de CSV. Cela facilite
leur lecture que ce soit par nous, développeurs, pour identifier si les valeurs semblent "cohérente" lors du développement, mais également
...
en \textit{R}, la lecture d'un fichier type CSV se fait facilement et de manière directe. Si l'on voulait ajouter des visualisations avec un autre logiciel,
il serait simple de lire les données sur celui-ci puisque les fichiers CSV sont massivement supportés. Cependant, ce choix d'implémentation aura un impact direct
sur la taille et le temps nécessaire à écrire les snapshots. 
\FloatBarrier
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{output_snapshot_enabled.png}
    \caption{Comparaison de la durée d'un run avec ou sans snaphots}
    \label{fig:snap_enabled}
\end{figure}
La figure \ref{fig:snap_enabled} montre donc la différence entre un run avec et sans snapshots. On peut voir que l'impact est énorme,
ici, on ne parle même pas du temps total de l'application avec la visualisation mais seulement de la simulation et de l'écriture des snapshhots. Pour la plus grande taille, écrire des snapshots
toutes les 50 itérations multiplie au moins par 20 la durée d'exécution de la simulation.
 Comme le but du projet est de produire et comparer avec un workflow in-situ, les snapshots serviront à 
...
C'est pour cela que nous avons ajouté l'option \textit{--save-interval [interval]} permettant de sauvegarder les snapshots toutes les \textit{interval} itérations.
...
afin de diminuer les coûts pour générer nos heatmaps. Pour ce faire nous avons ajouté une fonction \textit{generate\_snapshot\_slice} visible dans le Listing \ref{lst:slices}.
...
C'est un fichier contenant un header contenant le timestep et  les coordonnées des récepteus. Les coordonnées des récepteurs sont sous la forme \textit{coordX,coordY,coordZ} 
...
Nous avons ensuite des scripts R permettant de produire différentes statistiques et visualisations concernant les sismogrammes et snapshots.
...
Le coût de post-traitement est très élevé. Lors de nos expériences, nous avons fait des simulations avec des maillages allant de 50x50x50 à 120x120x120, comme le montre la Figure \ref{fig:post-treatment}.
 Le temps de traitement ad-hoc a mis plus d'une demi-heure sur les plus grandes tailles. Étant donné que les simulations réelles sont effectuées sur des maillages bien plus grand, le temps
d'analyse et de visualisation serait vraiment trop couteux.

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{post_treatment.png}
    \caption{Temps de post traitement en fonction de la taille du problème}
    \label{fig:post-treatment}
\end{figure}    
...
Un fichier est créé pour chaque sauvegarde de traitement, nous avons donc autant de fichiers que de fichiers de snapshots si nous mettons la même fréquence. En revanche, le poids des fichiers de traitement in-situ est
...
Pour éviter de sauvegarder des slices dans le seul but de les visualiser par la suite, nous avons ajouté la fonction \textit{export\_ppm\_slice}. Celle-ci
nous évite de devoir repasser sur les slices en \textit{ad-hoc} et donc de devoir re-charger un fichier pour seulement générer une visualisation. Le type de  visualisation
...
Si l'on compare cette nouvelle fonction avec la précédente, on peut voir que l'on doit parcourir le maillage deux fois. En effet, pour obtenir une échelle de la pression
et donc pouvoir ajouter la bonne couleur à chaque pixel, nous devons d'abord trouver les valeurs minimum et maximum pour générer la palette de couleurs. On doit également stocker
une deuxième fois les points du maillage pour notre représentation en pixel, ce qui peut avoir un coût en mémoire supplémentaire non négligeable si l'on travaille sur des grands domaines.
...
ce dossier par défaut pour éviter des temps d'écriture plus long dû au système de fichiers du CREMO.
\subsection{Comparaison en temps}
Le premier point que nous souhaitions comparer entre toutes nos versions était le temps mis pour différentes tailles données. Afin de pouvoir
réaliser un grand nombre d'expériences, nous nous sommes restreint à des tailles de maillage ne dépassant pas les 150x150x150. De plus, pour toutes les 
...
D'après les expériences que nous avons faites, on remarque que certaines versions ne pourraient pas passer à l'échelle. Il suffit de prendre la version utilisant
les snapshots complets pour faire de la visualisation pour voir que l'on ne pourra pas travailler avec de grands domaines. En effet, la taille des snapshots grandit
très vite et l'on se retrouve avec des snapshots d'environ 1Go pour une taille de problème de seulement 150x150x150. De plus, avec cette version le temps d'exécution
du workflow ad-hoc augmente de manière significative même avec des petites tailles. Il est donc impossible d'utiliser cette version pour des problèmes de grande taille,
même avec beaucoup de ressources (elles ne seraient pas assez exploitée). 
Si l'on souhaite pouvoir re-effectuer des visualisations, l'export sous forme de slice (compressées ou non) semble être une bonne alternative. Elle ne force pas à
utiliser le format PPM, mais il faudra tout de même penser au fait que les slices peuvent atteindre de tailles importantes si la taille du problème est grande.

Enfin, la version in-situ semble être la plus scalable. En effet, cette version sollicite beaucoup moins les I/O que les autres. Quand on sait que les applications
HPC sont souvent memory bound, il semble plus intéressant de limiter le nombre d'I/O lorsque cela n'est pas nécessaire. Nous pensons que la version optimale devrait donc mixer
la version avec les snapshots, qui serait ici utilisée afin de faire du checkpoiting permettant de pouvoir reprendre la simulation à un point donné. Ces snapshots ne seraient donc 
pas générés à chaque itération, mais plutôt toutes les $K$ itérations afin de ne pas trop impacter les performances de l'application. Pour la visualisation, le workflow in-situ
semble idéal pour rendre le programme performant pour la génération des heatmaps. Cependant, l'impact étant limité sur les sismogrammes et les visualisations de celui-ci bien plus explicite
en ad-hoc, on pourrait continuer de les sauvegarder de la même manière que dans le premier workflow. 


\section{Conclusion}

Nous avons donc vu ici qu'il était nécessaire de faire un workflow in-situ si l'on souhaitait passer à l'échelle. Cependant, pour que celui-ci puisse produire
des visualisations comparable avec celles du workflow ad-hoc comme l'ajout des textes, statistiques descriptives sur la même image, il aurait fallu utiliser
des outils spécifiques à l'in-situ. On aurait par exemple pu utiliser Adios2 ou ParaView Catalyst pour créer des visualisations plus poussée tout en réduisant les 
coûts d'un workflow ad-hoc.

Pour améliorer les performances de nos workflows, il faudrait également passer par des sauvegardes en binaire plutôt qu'en texte comme évoqué dans la partie \ref{snapshots}.
On pourrait également faire les sauvegarde en parallèle du calcul afin de ne pas se retrouver avec des phases de calcul puis d'I/O qui se suivent. L'idée serait ici de déclancher
la sauvergarde de l'état tout en commençant le calcul de la prochaine itération.

Pour finir, un workflow ad-hoc reste toujours interressant sur certains points. Tout d'abord il permet de bien séparer la partie visualisation de la partie calcul du code.
Cela rend plus facile le développement en parallèle de ces briques de l'application, puisqu'il n'y a qu'à se fixer un format commun avant de pouvoir développer les deux parties
de manières indépendantes. L'ad-hoc pourrait aussi permettre de ré-utiliser plus facilement un système de visualisation sur un autre code de calcul, car l'adaptation serait plus rapide 
(et donc moins couteuse).
\end{document}