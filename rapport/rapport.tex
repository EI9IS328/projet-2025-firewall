...
du workflow ad-hoc augmente de manière significative même avec des petites tailles. Il est donc impossible d'uitliser cette version pour des problème de grande taille,
même avec beaucoup de ressources (elles ne seraient pas assez exploitée). 
Si l'on souhaite pouvoir re-effectuer des visualisations, l'export sous forme de slice (compréssée ou non) semble être une bonne alternative. Elle ne force pas à
utiliser le format PPM, mais il faudra tout de même penser au fait que les slices peuvent atteindre de tailles importante si la taille du problème est grande.

Enfin, la version in-situ semble être la plus scalable. En effet, cette version solicite beaucoup moins les I/O que les autres. Quand on sait que les applications
HPC sont souvent memory bound, il semble plus interressant de limiter le nombre d'I/O lorsque cela n'est pas nécessaire. Nous pensons que la version optimale devrait donc mixer
...
pas générés à chaque itérations mais plutôt toutes les $K$ itérations afin de ne pas trop impacter les performances de l'application. Pour la visualisation, le workflow in-situ
...
en ad-hoc, on pourrait continuer de les sauvergader de la même manière que dans le premier workflow. 
